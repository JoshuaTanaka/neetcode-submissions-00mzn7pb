    ...
    public boolean isValidSudoku(char[][] board) {
        ...
        for(int i =0; i < board.length; i++){
            for(int j=0; j< board[0].length; j++){
                ...
                if(
                    horizontal.computeIfAbsent(i, k -> new HashSet<>()).contains(temp) ||
                    vertical.computeIfAbsent(j, k -> new HashSet<>()).contains(temp)   ||
                    square.computeIfAbsent((i / 3) * 3 + (j / 3), k -> new HashSet<>()).contains(temp)
                ){
                    return false;
                }  
